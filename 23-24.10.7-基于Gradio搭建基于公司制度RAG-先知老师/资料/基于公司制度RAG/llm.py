...
# 正常聊天指令
normal_system_prompt = "你叫瓜皮，一个帮助人们解答各种问题的助手。"
...
class MyKnowledge:
    # 向量化模型
    __embeddings = HuggingFaceBgeEmbeddings(
    ...
    def load_knowledge(self):
        ...
        for file in os.listdir(KNOWLEDGE_DIR):
            ...
            print("loader:", loader)
            self.__retrievers[collection_name] = create_indexes(
                collection_name, loader, self.__embeddings
            )
...
class MyLLM(MyKnowledge):
    ...
    def get_chain(self, collection, model, max_length, temperature):
        ...
        if collection:
            retriever = self.get_retrievers(
                collection
            )  # 如果有知识库，调用 get_retrievers 方法检索相应的知识库
            print("retriever:", retriever)

        # 只保留3个记录
        print(
            "len:",
            self.__chat_history.messages,
            "####:",
            len(self.__chat_history.messages),
        )
    ...
    def invoke(
        self, question, collection, model="gpt-3.5-turbo", max_length=256, temperature=1
    ):
