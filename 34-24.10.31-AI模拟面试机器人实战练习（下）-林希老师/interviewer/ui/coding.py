...
def send_request(
    ...
    # chat_history.append({
    #     'role': 'system', 
    #     'content': 'Please answer in a casual, conversational tone. Keep it simple and straightforward—no need for complex explanations or any special symbols.'
    # })
    # print("chat_history:")
    # print(chat_history)
    # print("==========================================")
...
def get_problem_solving_ui(
    ...
    with gr.Tab("Interview", render=False, elem_id=f"tab") as problem_tab:
        ...
        ).then(fn=tts.read_chat_message, inputs=[reply], outputs=[audio_output]
        ).then(fn=lambda: (np.array([], dtype=np.int16), "", False), outputs=[audio_buffer, hidden_text, is_transcribing]).then(
            fn=lambda: gr.update(visible=True), outputs=[audio_input]
        )
